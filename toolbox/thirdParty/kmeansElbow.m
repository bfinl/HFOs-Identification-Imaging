...
%%% Thomas.sauter@uni.lu
 ...
 DistanceVar ='sqeuclidean';   % 'sqeuclidean'; 'cosine'
...
if nargin>4, DistanceVar=cell2mat(varargin(4)); end

D=zeros(ToTest,1); %initialize the results matrix
for c=1:ToTest %for each sample
    [~,~,dist]=kmeans(X,c,'emptyaction','drop', 'Distance',DistanceVar); %compute the sum of intra-cluster distances
    tmp=sum(dist); %best so far
    
    for cc=2:Repeats %repeat the algo
        [~,~,dist]=kmeans(X,c,'emptyaction','drop', 'Distance',DistanceVar);
...
[IDX,C,SUMD]=kmeans(X,K,'Replicates',Repeats,'Distance',DistanceVar); %now rerun one last time with the optimal number of clusters
